    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 text-foreground">
      ...
      <div className="relative mb-8 rounded-2xl overflow-hidden bg-gradient-to-br from-card via-card to-blue-955/30 border border-border px-6 py-8 sm:px-10 sm:py-10">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative z-10">
          <div>
            <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-blue-500/10 border border-blue-500/20 text-blue-400 text-[10px] font-bold tracking-wider uppercase mb-3">
              <ShieldCheck className="h-3 w-3" /> Authorized Retailer
            </div>
            ...
            <p className="text-sm text-muted-foreground leading-relaxed max-w-xl">
              {brand?.description || `Explore genuine ${displayName} devices with official manufacturer warranty, fast islandwide shipping, and 0% interest 3-month installments with Koko & Mintpay.`}
            </p>
          </div>

          {brand?.logo && (
            <div className="h-20 w-36 relative shrink-0 rounded-xl bg-neutral-900/60 border border-border/80 p-3 flex items-center justify-center">
              <Image
                src={brand.logo}
      ...
      <div className="flex items-center justify-between gap-4 py-3 mb-6 border-b border-border/60">
        <p className="text-sm text-muted-foreground">
          Showing <span className="font-bold text-foreground tabular-nums">{products.length}</span> products from <span className="capitalize font-semibold text-foreground">{displayName}</span>
        </p>
      </div>
