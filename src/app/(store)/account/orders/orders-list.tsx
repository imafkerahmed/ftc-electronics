...
import { Package, Truck, CheckCircle2, Clock, AlertCircle, FileText, Loader2 } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { downloadCustomerInvoicePdfAction } from '@/app/actions/checkout';
...
export default function OrdersList({ initialOrders, initialError }: OrdersListProps) {
  ...
  return (
    <div className="space-y-6 text-foreground">
      ...
      {downloadErr && (
        <div role="alert" className="flex items-center gap-2 rounded-xl bg-destructive/10 border border-destructive/20 p-3 text-xs font-semibold text-destructive animate-in fade-in">
      ...
      {orders.length === 0 ? (
        ...
        <div className="space-y-6">
          {orders.map((order) => {
            ...
            const isPaid = order.is_paid === true || order.payment_status === 'paid' || order.paymentStatus === 'paid' || order.status === 'delivered' || order.status === 'completed';
                    ...
                    {isPaid && (
                      <Button
                        ...
                        onClick={async () => {
                          ...
                          setDownloadErr(null);
                          try {
                            const res = await downloadCustomerInvoicePdfAction(order.id);
                            if (res.success && res.pdfBase64) {
                              ...
                              a.href = url;
                              a.download = res.filename || `FTC-Invoice-${order.orderId || order.id}.pdf`;
                              document.body.appendChild(a);
                              a.click();
                              document.body.removeChild(a);
                              setTimeout(() => URL.revokeObjectURL(url), 1000);
                            } else {
                              setDownloadErr(res.error || 'Failed to download invoice PDF.');
                            }
                          } catch (pdfErr: any) {
                            setDownloadErr(pdfErr.message || 'Error downloading PDF.');
                          } finally {
                            setDownloadingId(null);
                          }
                        }}
                        className="h-7 text-xs px-2.5 font-medium border-blue-500/30 text-blue-600 hover:bg-blue-500/10 flex items-center gap-1.5 cursor-pointer"
                        title="Download Official Paid Invoice PDF"
                      >
                        {downloadingId === order.id ? <Loader2 className="h-3 w-3 animate-spin" /> : <FileText className="h-3 w-3" />}
                        Invoice PDF
                      </Button>
                    )}
