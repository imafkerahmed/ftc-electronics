...
async function fetchNewArrivals(): Promise<Product[]> {
  const res = await fetch('/api/products/collection/new-arrivals', { cache: 'no-store' });
  if (!res.ok) throw new Error('Failed to fetch new arrivals');
  return res.json();
}

export default function NewArrivalsClient({ initialProducts }: { initialProducts: Product[] }) {
  const { data: products = initialProducts } = useQuery<Product[]>({
    queryKey: [...productKeys.lists(), 'collection', 'new-arrivals'],
