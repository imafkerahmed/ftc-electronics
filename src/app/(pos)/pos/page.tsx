...
import { getPosSession, clearPosSession, isPosSessionValid } from '@/lib/pos-session';
...
export default function PosPage() {
  ...
  const handleAddToCart = useCallback((newItem: Omit<PosCartItem, 'quantity' | 'itemDiscount' | 'lineTotal'>) => {
    setCart((prev) => {
      ...
      if (isSpecificUnit) {
        ...
        const item: PosCartItem = { ...newItem, quantity: 1, itemDiscount: 0, lineTotal: newItem.unitPrice };
        return [...prev, item];
      }

      const maxAllowed = typeof newItem.countInStock === 'number' ? newItem.countInStock : 9999;
      if (existing) {
        ...
        return prev.map((i) =>
          i.key === newItem.key
            ? { ...i, quantity: newQty, lineTotal: calcLineTotal({ ...i, quantity: newQty }) }
            : i
        );
      }
      const item: PosCartItem = { ...newItem, quantity: 1, itemDiscount: 0, lineTotal: newItem.unitPrice };
    ...
    setCart((prev) =>
      prev
        .map((i) => {
          ...
          const maxAllowed = typeof i.countInStock === 'number' ? i.countInStock : 9999;
          const qty = Math.min(maxAllowed, Math.max(1, i.quantity + delta));
          return { ...i, quantity: qty, lineTotal: calcLineTotal({ ...i, quantity: qty }) };
        })
  ...
  const handleUpdateDiscount = useCallback((key: string, discount: number) => {
    setCart((prev) =>
      prev.map((i) =>
        i.key === key ? { ...i, itemDiscount: discount, lineTotal: calcLineTotal({ ...i, itemDiscount: discount }) } : i
      )
    );
  }, []);
  ...
  const billData = (() => {
    ...
    const itemDiscountsTotal = cart.reduce((sum, i) => sum + i.itemDiscount * i.quantity, 0);
    ...
    const discount = itemDiscountsTotal + globalDiscountAmt;
    ...
    <div className="flex flex-col h-screen bg-background overflow-hidden">
      ...
      <header className="h-14 shrink-0 border-b border-border bg-card/60 backdrop-blur-md flex items-center justify-between px-6 gap-4 relative z-10">
        <div className="flex items-center gap-3">
          <Link href="/pos" className="text-lg font-bold tracking-wider flex items-center">
            <span className="bg-gradient-to-r from-blue-500 to-indigo-500 bg-clip-text text-transparent font-extrabold">FTC</span>
            <span className="text-muted-foreground font-light mx-1.5">|</span>
            <span className="text-sm font-semibold text-foreground/80 tracking-normal">Electronics</span>
          </Link>
        ...
        <div className="flex items-center gap-2">
          <Link href="/pos/history" className={buttonVariants({ variant: 'outline', size: 'sm' })}>
            <History className="h-3.5 w-3.5" /> History
          </Link>
          {cart.length > 0 && (
            <Button variant="outline" size="sm" onClick={handleClearCart}>
              <RefreshCw className="h-3.5 w-3.5" /> Clear Cart
            </Button>
          )}
          <Button
            variant="outline"
            size="sm"
            onClick={() => { clearPosSession(); setSession(null); }}
            title="Switch cashier"
          >
            <User className="h-3.5 w-3.5 text-blue-500" />
            <span className="max-w-[100px] truncate font-semibold">{session.name}</span>
        ...
        <div className="w-[340px] shrink-0 flex flex-col p-4 gap-0 overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-2 shrink-0">
            <span className="text-xs font-bold text-foreground">
              Cart {cart.length > 0 && <span className="text-blue-500">({cart.length})</span>}
            </span>
          </div>
