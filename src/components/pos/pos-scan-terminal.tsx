...
import {
  ScanLine, Barcode, CheckCircle2, AlertCircle, X, ChevronDown, Grid3x3, Plus, Search, Loader2,
} from 'lucide-react';
...
export default function PosScanTerminal({
  ...
  const loadProducts = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        ...
        const cats = ['All', ...new Set<string>((data.products as Product[]).map((p: Product) => p.category).filter(Boolean))];
        setCategories(cats);
  ...
  const flashFeedback = (status: 'ok' | 'error') => {
    ...
    if (!el) return;
    el.classList.remove('bg-emerald-500/10', 'bg-red-500/10', 'border-emerald-500/40', 'border-red-500/40', 'border-transparent');
    el.classList.add(status === 'ok' ? 'bg-emerald-500/10' : 'bg-red-500/10');
    el.classList.add(status === 'ok' ? 'border-emerald-500/40' : 'border-red-500/40');
    setTimeout(() => {
      el.classList.remove('bg-emerald-500/10', 'bg-red-500/10', 'border-emerald-500/40', 'border-red-500/40');
  ...
  const addProductToCart = (p: Product, skipFlash = false) => {
    ...
    const availUnit = p.availableUnits?.find(
      (u) => !cartItems.some((c) => c.unitBarcode === u.barcode || c.unitId === u.id)
    );
  ...
  const handleScan = async (q: string) => {
    ...
    try {
      const res = await fetch(`/api/pos/scan?q=${encodeURIComponent(q.trim())}`);
      const data = await res.json();

      if (res.ok && data.success && data.data) {
        const item = data.data;
        const uniqueKey = item.unitBarcode ? `${item.productId}-${item.unitBarcode}` : item.productId;
      ...
      >
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-xs font-black text-foreground uppercase tracking-wider">Scan Terminal</p>
              <p className="text-[10px] text-muted-foreground">Scan barcode or type product name / SKU</p>
            </div>
          </div>
          <span className="text-[10px] text-muted-foreground font-mono bg-muted px-2 py-0.5 rounded-md">Press / to focus</span>
        </div>
        ...
        <div className="relative">
          ...
          <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1.5">
            {scanning && <Loader2 className="h-4 w-4 animate-spin text-blue-400" />}
            {scanInput && !scanning && (
              <button
                onClick={() => { setScanInput(''); setShowSuggestions(false); }}
          ...
          {showSuggestions && searchSuggestions.length > 0 && (
            ...
            >
              <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider px-3 pt-2 pb-1">
                {searchSuggestions.length} match{searchSuggestions.length !== 1 ? 'es' : ''} — click to add to cart
              </p>
              <div className="max-h-64 overflow-y-auto divide-y divide-border">
                {searchSuggestions.map((p) => {
                  const alreadyInCart = cartItems.some((c) => c.productId === p.id);
                  return (
                    <button
                      key={p.id}
                      type="button"
                    ...
                    >
                      ...
                      <div className="shrink-0 text-right">
                        <p className="text-xs font-black text-foreground">{fmt(p.price, currency)}</p>
                        <p className={`text-[10px] font-semibold ${p.countInStock > 5 ? 'text-muted-foreground' : p.countInStock > 0 ? 'text-amber-500' : 'text-red-500'}`}>
                          {p.countInStock === 0 ? 'Out of stock' : `Stock: ${p.countInStock}`}
                        </p>
                      </div>
                      {/* Already in cart badge */}
                      {alreadyInCart && (
                        <span className="shrink-0 text-[9px] font-bold bg-emerald-500/10 text-emerald-600 border border-emerald-500/20 px-1.5 py-0.5 rounded-md">In cart</span>
                      )}
                    </button>
                  );
                })}
        ...
        {/* Last scan feedback */}
        {lastScan && (
          <div className={`flex items-center justify-between gap-2 px-3 py-2 rounded-xl border text-xs font-semibold transition-all ${
            lastScan.status === 'ok'
            ...
            <button onClick={() => setLastScan(null)} className="shrink-0 hover:opacity-70">
          ...
          <div className="space-y-1 max-h-[130px] overflow-y-auto">
            {scanLog.map((ev) => (
              ...
              >
                ...
                {ev.unitBarcode && (
                  <span className="font-mono text-[10px] text-muted-foreground shrink-0">{ev.unitBarcode}</span>
                )}
                <span className="text-[9px] text-muted-foreground shrink-0 font-mono">
                  {new Date(ev.ts).toLocaleTimeString('en-LK', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
                </span>
              </div>
            ))}
          </div>
      ...
      <div className="shrink-0">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            {!productsLoaded && <Loader2 className="h-3 w-3 animate-spin text-muted-foreground" />}
          </div>
          <ChevronDown className={`h-3.5 w-3.5 text-muted-foreground transition-transform ${showCatalog ? 'rotate-180' : ''}`} />
        </button>
      </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {filteredCatalog.map((p) => (
                  ...
                  >
                    ...
                    <p className="text-sm font-black text-foreground mt-1">{fmt(p.price, currency)}</p>
                    {p.countInStock > 0 && (
