...
import { X, ArrowRight, AlertCircle, CheckCircle2, Eye, EyeOff, Loader2 } from 'lucide-react';
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      const { error: oauthError } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${window.location.origin}/auth/callback?next=${encodeURIComponent(onSuccessRedirect || '/')}`,
        },
      });
      ...
      try {
        ...
        if (res.success) {
          if (res.requiresConfirmation) {
            setMessage(res.message || 'Confirmation link sent to your email. Please check your inbox.');
            setMode('signin');
          } else {
            window.dispatchEvent(new Event('auth-change'));
            onClose();
