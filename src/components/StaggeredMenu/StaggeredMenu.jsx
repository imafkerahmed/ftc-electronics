...
export const StaggeredMenu = ({
  ...
  const [currentView, setCurrentView] = useState({ type: 'main', activeItem: null });
  ...
  const transitionToView = useCallback((targetView) => {
    ...
    gsap.to(labels, {
      ...
      onComplete: () => {
        ...
        setTimeout(() => {
          const newLabels = Array.from(panel.querySelectorAll('.sm-panel-itemLabel'));
  ...
  const handleItemClick = useCallback((e, it) => {
    if (it.isBack) {
      e.preventDefault();
  ...
  const activeItemsList = currentView.type === 'main' 
    ? items 
          ...
          <ul className="sm-panel-list" role="list" data-numbering={displayItemNumbering || undefined}>
            {renderList.length ? (
              renderList.map((it, idx) => (
                <li className="sm-panel-itemWrap" key={(it.isBack ? 'back' : it.label) + idx}>
                  <Link 
                    className={'sm-panel-item' + (it.isBack ? ' sm-item-back' : '')} 
                    href={it.link} 
