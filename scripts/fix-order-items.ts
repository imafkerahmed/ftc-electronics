...
async function main() {
  ...
  const { data: orderRecord, error } = await adminSupabase
    .from('orders')
    .select('*')
    .or(`order_id.eq.${targetOrderId},id.eq.${targetOrderId}`)
    .single();
  ...
  console.log(`[fix-order-items] Order found: ID=${orderRecord.id}, OrderNo=${orderRecord.order_id}, Status=${orderRecord.status}`);
  ...
  if (!isApply) {
    console.log('[fix-order-items] DRY RUN complete. Run with --apply to apply updates.');
    return;
  }

  const { error: updateError } = await adminSupabase
    .from('orders')
    .update({ items: consolidated })
    .eq('id', orderRecord.id);

  if (updateError) {
    console.error(`❌ [fix-order-items] Error updating order:`, updateError);
  } else {
    console.log(`✅ [fix-order-items] Successfully updated order ${orderRecord.order_number || orderRecord.id}.`);
  }
}
